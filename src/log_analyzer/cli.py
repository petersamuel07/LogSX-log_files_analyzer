"""Command-line interface for LogSX — Log Files Analyzer.

This module owns the whole CLI: argument parsing, the command handlers, and
the process exit code. The repo-root ``main.py`` is only a shim that calls
:func:`main` here, so all three of these are equivalent:

    logsx pipeline data/sample/sample_5000.log     (installed console script)
    python -m log_analyzer pipeline ...            (module entry point)
    python main.py pipeline ...                    (repo-root shim)

Typical workflow
----------------
    logsx init-db
    logsx generate-sample --num-lines 5000
    logsx pipeline data/sample/sample_5000.log

Or run each stage independently:

    logsx ingest data/logs                # file or directory
    logsx analyze
    logsx report
    logsx charts

Run ``logsx -h`` or ``logsx <command> -h`` for full option lists.

Command handlers import their services lazily, inside the function body, so
that a fast command (``logsx -h``) doesn't pay the import cost of pandas,
